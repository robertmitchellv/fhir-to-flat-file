...
```{r}
health_check <- "http://localhost:8082/fhir-converter/"
fhir_converter <- "http://localhost:8082/fhir-converter/convert-to-fhir"
```
...
save_json_response <- function(response, file_id, output_dir) {
  ...
  json_file_path <- path(output_dir, paste0(file_id, "_bundle.json"))
  write_json(json_response, json_file_path, pretty = TRUE)
...
```{r}
process_files(data_dir = ecr_data_directory, output_dir = fhir_bundle_directory, api_url = fhir_converter)
```
