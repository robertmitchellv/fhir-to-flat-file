...
```{r}
health_check <- "http://localhost:8085/"
message_parser <- "http://localhost:8085/parse_message"
```
...
So we can see some different parsing schemas that are available to use. We can create our own and uplod them to the API to use. For now, we'll stick with the `ecr.json` parsing schema.
